...
class Movie():
    ...
    # Defining API key at Class level to avoid repeating it in code base
    ...
    def get_movie_id(self, movie_title):
        ...
        The themoviedb.org API can search for movies by title, but this function
        ...
        # than one result. The following line sorts the results by the number of
        # themoviedb.org user votes.
    ...
    def get_video_key(self, response_dict):
        """This method extracts the Youtube video key from the movie information
        ...
        # Many movies return multiple available videos. The following line sorts
        # the results by the size (resolution) of the videos.
        sorted_dict = sorted(response_dict['videos']['results'],
                             key=itemgetter('size'),
                             reverse=True)

        # Not all available videos are trailers. The following iterates over the
        # the results selecting the first of the sorted videos that is a trailer.
        # The second conditional selects only videos below 1080 resolution.
        # Several 1080 HD videos were restricted to only play on Youtube and
        # thus could not be embedded. This is a hacky way of avoiding this issue.
        # TODO: find a way to check embed-ability via Youtube API
